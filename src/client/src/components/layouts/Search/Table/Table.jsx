...
class Table extends React.Component {
  ...
  render() {
    ...
    return (
      <div>
        {itemsToDisplay}
        <button onClick={() => this.setPage(this.state.currentPage - 1)} disable={this.state.currentPage === 1} >&lt;</button>
        <label>{this.props.lang.page}: {this.state.currentPage}</label>
        <button onClick={() => this.setPage(this.state.currentPage + 1)} disabled={this.state.currentPage === pageCount}>&gt;</button>
      </div>
    )
  }
}
